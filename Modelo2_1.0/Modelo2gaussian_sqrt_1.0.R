# =============================================================================
# ANÁLISE DE PRODUTIVIDADE — GLMMTMB GAUSSIAN SQRT
...
# =============================================================================
# MODELO 2 — MÉDIA (Gaussian Sqrt)
# =============================================================================

mod_gaussian_sqrt_media <- glmmTMB(
  GY_media ~ Ambiente + (1 | GEN) + (1 | GEN:Ambiente),
  data   = dados_sumarizados_v3,
  family = gaussian(link = "sqrt")
)

resultado1              <- ajustar_categorica(dados_sumarizados_v3, "Ambiente", mod_gaussian_sqrt_media)
mod_gaussian_sqrt_media <- resultado1$modelo_atualizado

summary(mod_gaussian_sqrt_media)

# ── Diagnóstico de resíduos ───────────────────────────────────────────────────
residuos_dharma_media <- simulateResiduals(mod_gaussian_sqrt_media)
...
# =============================================================================
# MODELO 2 — MEDIANA (Gaussian Sqrt)
# =============================================================================

mod_gaussian_sqrt_mediana <- glmmTMB(
  GY_mediana ~ Ambiente + (1 | GEN) + (1 | GEN:Ambiente),
  data   = dados_sumarizados_v3,
  family = gaussian(link = "sqrt")
)

resultado2                <- ajustar_categorica(dados_sumarizados_v3, "Ambiente", mod_gaussian_sqrt_mediana)
mod_gaussian_sqrt_mediana <- resultado2$modelo_atualizado

summary(mod_gaussian_sqrt_mediana)

# ── Diagnóstico de resíduos ───────────────────────────────────────────────────
residuos_dharma_mediana <- simulateResiduals(mod_gaussian_sqrt_mediana)
...
ranef(mod_gaussian_sqrt_mediana)
...
rank_media <- ranef(mod_gaussian_sqrt_media)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  arrange(desc(BLUP)) %>%
  mutate(Rank = row_number(), Modelo = "Média")

rank_mediana <- ranef(mod_gaussian_sqrt_mediana)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  arrange(desc(BLUP)) %>%
...
plot_genotipos_redgreen(rank_media,   "Ranqueamento dos Genótipos — Média Gaussian Sqrt")
plot_genotipos_redgreen(rank_mediana, "Ranqueamento dos Genótipos — Mediana Gaussian Sqrt")
