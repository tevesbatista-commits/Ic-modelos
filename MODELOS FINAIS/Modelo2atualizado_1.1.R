# =============================================================================
# ANÁLISE DE PRODUTIVIDADE — GLMMTMB GAUSSIAN LOG
# Efeito fixo: Ambiente | Efeitos aleatórios: Genótipo + GEN:Ambiente
# =============================================================================
...
# =============================================================================
# MODELO 1 — MÉDIA (Gaussian Log)
# =============================================================================

mod_gaussian_media <- glmmTMB(
  GY_media ~ Ambiente + (1 | GEN) + (1 | GEN:Ambiente),
  data   = dados_sumarizados_v3,
  family = gaussian(link = "log")
)

resultado1         <- ajustar_categorica(dados_sumarizados_v3, "Ambiente", mod_gaussian_media)
mod_gaussian_media <- resultado1$modelo_atualizado

summary(mod_gaussian_media)

# ── Diagnóstico de resíduos ───────────────────────────────────────────────────
residuos_dharma_media <- simulateResiduals(mod_gaussian_media)
...
# =============================================================================
# MODELO 2 — MEDIANA (Gaussian Log)
# =============================================================================

mod_gaussian_mediana <- glmmTMB(
  GY_mediana ~ Ambiente + (1 | GEN) + (1 | GEN:Ambiente),
  data   = dados_sumarizados_v3,
  family = gaussian(link = "log")
)

resultado2           <- ajustar_categorica(dados_sumarizados_v3, "Ambiente", mod_gaussian_mediana)
mod_gaussian_mediana <- resultado2$modelo_atualizado

summary(mod_gaussian_mediana)

# ── Diagnóstico de resíduos ───────────────────────────────────────────────────
residuos_dharma_mediana <- simulateResiduals(mod_gaussian_mediana)
...
media_geral_media   <- fixef(mod_gaussian_media)$cond["(Intercept)"]
media_geral_mediana <- fixef(mod_gaussian_mediana)$cond["(Intercept)"]

# ── Rank Genótipos — Média ────────────────────────────────────────────────────
# Nota: ranef()$cond$GEN extrai apenas o efeito principal do Genótipo.
rank_media <- ranef(mod_gaussian_media)$cond$GEN %>%
...
rank_mediana <- ranef(mod_gaussian_mediana)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  mutate(
    # FÓRMULA EXATA SOLICITADA
...
emm_media <- as.data.frame(
  emmeans(mod_gaussian_media, specs = ~Ambiente, type = "response")
) %>%
...
emm_mediana <- as.data.frame(
  emmeans(mod_gaussian_mediana, specs = ~Ambiente, type = "response")
) %>%
  arrange(desc(response)) %>%
  mutate(
    Rank    = row_number(),
...
plot_genotipos_redgreen <- function(df, titulo) {
  ggplot(df, aes(x = reorder(GEN, BLUP_Perc), y = BLUP_Perc, fill = BLUP_Perc > 0)) +
    ...
    labs(
      title    = titulo,
      subtitle = "BLUPs do efeito aleatório principal (1|GEN)",
      x        = NULL,
      y        = "Ganho Genético (%)",
      fill     = NULL
    ) +
...
plot_genotipos_redgreen(rank_media,   "Ranqueamento dos Genótipos — Média Gaussian Log")
plot_genotipos_redgreen(rank_mediana, "Ranqueamento dos Genótipos — Mediana Gaussian Log")


# =============================================================================
# HEATMAP — RANK COMPARATIVO MÉDIA vs MEDIANA (GAUSSIAN LOG)
# =============================================================================
...
ggplot(rank_comparativo, aes(x = Modelo, y = reorder(GEN, BLUP_Perc), fill = BLUP_Perc)) +
  ...
  theme_bw(base_size = 12) +
  labs(
    title = "Heatmap — Ganho Genético (%) por Genótipo (Gaussian Log)",
    x     = "Modelo",
    y     = "Genótipo"
  ) +
  theme(
