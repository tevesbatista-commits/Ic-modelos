...
# =============================================================================
# ANÁLISE DE PRODUTIVIDADE — GAMMA IDENTIDADE (local_estado + Year_Date)
# =============================================================================

# ── MODELO 3 MÉDIA ──────────────────────────────────────────────────────────────
mod_gamma_id_local_media <- glmmTMB(
  GY_media ~ local_estado + Year_Date + (1 | GEN),
  data   = dados_sumarizados_v3,
  family = Gamma(link = "identity")
)
summary(mod_gamma_id_local_media)

residuos_dharma_media <- simulateResiduals(mod_gamma_id_local_media)
...
# ── MODELO 3 MEDIANA ────────────────────────────────────────────────────────────
mod_gamma_id_local_mediana <- glmmTMB(
  GY_mediana ~ local_estado + Year_Date + (1 | GEN),
  data   = dados_sumarizados_v3,
  family = Gamma(link = "identity")
)
summary(mod_gamma_id_local_mediana)

residuos_dharma_mediana <- simulateResiduals(mod_gamma_id_local_mediana)
par(mar = c(3, 3, 1, 1), oma = c(0, 0, 0, 0))
...
# =============================================================================
rank_media <- ranef(mod_gamma_id_local_media)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  arrange(desc(BLUP)) %>%
  mutate(Rank = row_number(), Modelo = "Média")

rank_mediana <- ranef(mod_gamma_id_local_mediana)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  arrange(desc(BLUP)) %>%
...
plot_genotipos_redgreen(rank_media,   "Ranqueamento dos Genótipos — Média Gamma Identidade")
plot_genotipos_redgreen(rank_mediana, "Ranqueamento dos Genótipos — Mediana Gamma Identidade")
