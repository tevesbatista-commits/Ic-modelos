# =============================================================================
# ANÁLISE DE PRODUTIVIDADE — GLMMTMB INVERSE GAUSSIAN (LINK LOG)
...
# =============================================================================
# MODELO 1 — MÉDIA (Inverse Gaussian Log)
# =============================================================================
mod_invgauss_media <- glmmTMB(
  GY_media ~ Ambiente + (1 | GEN),
  data   = dados_sumarizados_v3,
  family = inverse.gaussian(link = "log")  # <-- corrigido
)
resultado1         <- ajustar_categorica(dados_sumarizados_v3, "Ambiente", mod_invgauss_media)
mod_invgauss_media <- resultado1$modelo_atualizado
summary(mod_invgauss_media)

residuos_dharma_media <- simulateResiduals(mod_invgauss_media)
...
# =============================================================================
# MODELO 1 — MEDIANA (Inverse Gaussian Log)
# =============================================================================
mod_invgauss_mediana <- glmmTMB(
  GY_mediana ~ Ambiente + (1 | GEN),
  data   = dados_sumarizados_v3,
  family = inverse.gaussian(link = "log")  # <-- corrigido
)
resultado2           <- ajustar_categorica(dados_sumarizados_v3, "Ambiente", mod_invgauss_mediana)
mod_invgauss_mediana <- resultado2$modelo_atualizado
summary(mod_invgauss_mediana)

residuos_dharma_mediana <- simulateResiduals(mod_invgauss_mediana)
par(mar = c(1, 1, 1, 1), oma = c(0, 0, 0, 0))
...
# =============================================================================
rank_media <- ranef(mod_invgauss_media)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  arrange(desc(BLUP)) %>%
  mutate(Rank = row_number(), Modelo = "Média")

rank_mediana <- ranef(mod_invgauss_mediana)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  arrange(desc(BLUP)) %>%
...
plot_genotipos_redgreen(rank_media,   "Ranqueamento dos Genótipos — Média Inverse Gaussian Log")
plot_genotipos_redgreen(rank_mediana, "Ranqueamento dos Genótipos — Mediana Inverse Gaussian Log")
?family
