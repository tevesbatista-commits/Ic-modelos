# =============================================================================
# ANÁLISE DE PRODUTIVIDADE — GLMMTMB GAUSSIAN (LINK LOG)
...
mod_gaussian_media <- glmmTMB(
  ...
  family = gaussian(link = "log")          # <-- mudança
)
...
ggplot(dados_sumarizados_v3, aes(x = Ambiente, y = GY_media, fill = Ambiente)) +
  ...
  labs(title = "Genótipos por Ambiente", x = "Ambiente", y = "Produtividade Média (kg/ha)") +
  theme(
...
ggplot(dados_sumarizados_v3, aes(x = Ambiente, y = GY_mediana, fill = Ambiente)) +
  ...
  theme_bw(base_size = 12) +
  labs(title = "Genótipos por Ambiente", x = "Ambiente", y = "Mediana de Produtividade (kg/ha)") +
  theme(
    axis.text.x     = element_text(angle = 45, vjust = 1, hjust = 1, size = 5),
    legend.position = "none",
    plot.title      = element_text(hjust = 0.5, face = "bold")
...
rank_media <- ranef(mod_gaussian_media)$cond$GEN %>%
  ...
  arrange(desc(BLUP)) %>%
  mutate(Rank = row_number(), Modelo = "Média")

rank_mediana <- ranef(mod_gaussian_mediana)$cond$GEN %>%
  rownames_to_column("GEN") %>%
  rename(BLUP = `(Intercept)`) %>%
  arrange(desc(BLUP)) %>%
  mutate(Rank = row_number(), Modelo = "Mediana")
...
plot_genotipos_redgreen <- function(df, titulo) {
  ggplot(df, aes(x = reorder(GEN, BLUP), y = BLUP, fill = BLUP > 0)) +
    geom_col(width = 0.65) +
    geom_text(aes(label = paste0("#", Rank)), hjust = hjust_blup(df$BLUP), size = 3.5) +
    scale_fill_manual(
