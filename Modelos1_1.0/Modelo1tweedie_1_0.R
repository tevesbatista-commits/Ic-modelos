...
testDispersion(residuos_dharma_media)   # CORRIGIDO: deve receber objeto DHARMa
testOutliers(residuos_dharma_media)     # CORRIGIDO: deve receber objeto DHARMa
...
testDispersion(residuos_dharma_mediana)  # CORRIGIDO: deve receber objeto DHARMa
testOutliers(residuos_dharma_mediana)    # CORRIGIDO: deve receber objeto DHARMa
...
plot_genotipos_redgreen <- function(df, titulo) {
  ggplot(df, aes(x = reorder(GEN, BLUP), y = BLUP, fill = BLUP > 0)) +
    ...
    scale_fill_manual(
      values = c("FALSE" = "#bc4749", "TRUE" = "#386641"), # Vermelho escuro vs Verde escuro
